...
export interface IteHandshakeKeys {
...
export async function generateHandshakeKeys(): Promise<IteHandshakeKeys> {
...
async function deriveFingerprint(keyBytes: Uint8Array): Promise<string> {
  const digest = new Uint8Array(await crypto.subtle.digest("SHA-256", asBufferSource(keyBytes)))
  const groups: string[] = []
  for (let i = 0; i < 8; i += 1) {
...
export async function deriveIteSession(
  privateKey: CryptoKey,
  ...
  const hkdfKey = await crypto.subtle.importKey("raw", asBufferSource(sharedBits), "HKDF", false, [
    "deriveBits",
    "deriveKey",
  ])
  ...
  const aesKey = await crypto.subtle.deriveKey(
    {
      name: "HKDF",
      hash: "SHA-256",
      salt: asBufferSource(encoder.encode(ITE_SALT)),
      info: asBufferSource(encoder.encode(ITE_INFO)),
    },
    hkdfKey,
...
function counterToNonce(counter: Uint8Array): Uint8Array {
